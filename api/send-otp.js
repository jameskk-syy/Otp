...
const sendOtp = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'OTP sent successfully' , status:'success', statusCode:200});
    } catch (error) {
        res.status(500).json({ message: 'Failed to send OTP', error: error.message , status:'error', statusCode:404});
    }
};
